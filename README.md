...
<p align="center">
  ...
  <a href="https://github.com/suryaprakash251201/nexora/blob/main/VERSION"><img src="https://img.shields.io/badge/version-1.11.0-5B8CFF?style=flat-square" alt="version 1.11.0" /></a>
...
# 3. Verify
curl -f http://localhost/healthz          # {"service":"nexora","status":"ok","version":"1.11.0"}
...
```bash
docker build -t nexora:1.11.0 --build-arg VERSION=1.11.0 .
...
| App | Stack | Version |
|-----|-------|---------|
| **Web** | React 19, Vite, Tailwind 4, Zustand, React Query, Motion | `1.11.0` — `web/` |
| **Desktop** | Tauri 2 wrapper around `web/dist` | `1.11.0` — `desktop/` |
...
VERSION               Single source of truth (1.11.0) — web/desktop/image derive from it
```
...
Versioning follows SemVer; current version in root [`VERSION`](VERSION) (`1.11.0`) — `web` and `desktop` are synced to it, `mobile` has an independent store version.
