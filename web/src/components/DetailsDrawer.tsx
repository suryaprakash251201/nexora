...
export default function DetailsDrawer({
  ...
  return (
    <aside className="w-80 shrink-0 border-l bg-surface-muted/30 h-full flex flex-col">
      <div className="h-14 flex items-center justify-between px-4 border-b">
        <span className="font-semibold">Details</span>
        <button onClick={onClose} className="p-1 rounded hover:bg-surface-muted"><X className="h-4 w-4" /></button>
      </div>
      {!path && (
      ...
      {path && data && (
        <div className="p-4 space-y-4 overflow-y-auto">
          ...
          <div className="flex flex-col gap-1 pt-2 border-t">
            <button onClick={onDownload} className="flex items-center gap-2 px-3 py-2 rounded-lg hover:bg-surface-muted text-sm">
            ...
            {canWrite && (
              <>
                <button onClick={onRename} className="flex items-center gap-2 px-3 py-2 rounded-lg hover:bg-surface-muted text-sm">
                  <Pencil className="h-4 w-4" /> Rename
                </button>
                <button onClick={onMove} className="flex items-center gap-2 px-3 py-2 rounded-lg hover:bg-surface-muted text-sm">
                  <Scissors className="h-4 w-4" /> Move
                </button>
                <button onClick={onCopy} className="flex items-center gap-2 px-3 py-2 rounded-lg hover:bg-surface-muted text-sm">
