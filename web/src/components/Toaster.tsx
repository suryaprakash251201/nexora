...
export default function Toaster() {
  ...
  return (
    <div className="fixed bottom-4 right-4 z-[60] flex flex-col gap-2 w-80 max-w-[calc(100vw-2rem)]">
      {toasts.map((t) => (
        <div key={t.id} className="toast-in flex items-start gap-2 bg-surface-elevated border rounded-lg p-3 shadow-md">
          ...
          <button onClick={() => dismiss(t.id)} className="text-content-muted hover:text-content">
            <X className="h-4 w-4" />
          </button>
        </div>
      ))}
