...
export default function Setup({ onSuccess }: { onSuccess: () => void }) {
  ...
  async function submit(e: React.FormEvent) {
    ...
    setBusy(true);
    try {
      const res = await authApi.setup({ username, email, password, display_name: displayName });
      if (res && res.token) {
    ...
    <div className="min-h-screen bg-background flex flex-col items-center justify-center p-4 relative overflow-hidden">
      ...
      <div className="w-full max-w-md animate-scale-in relative z-10">
        <div className="mb-8 text-center">
          <NexoraLogo size={80} idPrefix="setup" className="mx-auto mb-6 drop-shadow-[0_4px_12px_rgba(139,92,246,0.5)]" />
