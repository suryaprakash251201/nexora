      ...
      {kind === "video" && (
        <View style={styles.center}>
          <VideoPlayerComponent
            ...
            ext={item.extension}
            title={item.name}
            storageKey={`${item.root_id || rootId}:${item.path}`}
            onFallback={downloadAndOpen}
          />
        </View>
      )}
...
// ── Video ─────────────────────────────────────────────────────────────
// The full VLC-style player lives in src/components/VideoPlayer.tsx — it is
// rendered with the raw URL when the device can decode the container natively,
// and automatically falls back to the server transcode stream (H.264/AAC)
// otherwise or on playback errors.
