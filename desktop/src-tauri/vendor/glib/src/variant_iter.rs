...
impl<'a> VariantStrIter<'a> {
    ...
    fn impl_get(&self, i: usize) -> &'a str {
        unsafe {
            // SECURITY (GHSA-wrw7-89jp-8q8g / RUSTSEC-2024-0429): the
            // out-argument pointer MUST be passed as `&mut p`. Passing `&p`
            // mutated the pointer behind a shared reference, which optimizers
            // are free to discard, leaving `p` NULL and causing UB/crashes in
            // `CStr::from_ptr`. Backport of gtk-rs-core commit b5a4071e
            // (`glib: fix UB in VariantStrIter::impl_get`), released in 0.20.0.
